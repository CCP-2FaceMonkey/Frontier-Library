...
export function PlaceholderListPage({ section }: { section: SlugContentSection }) {
  ...
  return (
    <div className="app-placeholder-page">
      ...
      <div
        className={
          section === 'diaries' ? 'home-card-grid home-card-grid--diaries' : 'home-card-grid'
        }
      >
        {section === 'diaries' ? (
          <>
          ...
          items.map((item) => (
            <RouteSquareCard key={item.slug} item={item} to={`/${section}/${item.slug}`} />
          ))
        )}
      </div>
    </div>
